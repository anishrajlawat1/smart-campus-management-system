...
const StudentLayout = ({
  ...
  const studentModules = [
    ...
    { title: 'Routines', path: '/student/routines', icon: Calendar },
    { title: 'Attendance', path: '/student/attendance', icon: ClipboardCheck },
    { title: 'Exam Seats', path: '/student/exams', icon: FileText },
    { title: 'Notices', path: '/student/notices', icon: Bell },
    { title: 'Events', path: '/student/events', icon: Calendar },
    { title: 'Lost & Found', path: '/student/lost-found', icon: Users },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-slate-100">
      <aside className="w-64 bg-white border-r border-slate-200 p-6 flex flex-col">
        ...
        <div className="bg-slate-50 rounded-2xl px-3 py-4 mt-4 flex items-center gap-3">
          <img
            src={user?.profile_image || getFallbackImage(user?.name)}
            alt={user?.name || 'Student'}
            className="w-10 h-10 rounded-full object-cover border border-slate-200 hover:ring-2 hover:ring-indigo-500 transition bg-white"
          />
                ...
                <div className="max-h-96 overflow-y-auto">
                  {notifications.length > 0 ? (
                    ...
                    <div className="p-8 text-center">
                      ...
                      <p className="text-sm text-slate-500 font-medium mt-1">
                        Your student updates will appear here.
                      </p>
                    </div>
                  )}
                </div>
